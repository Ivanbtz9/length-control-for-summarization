...
results_dir = os.path.join(results_path,f"results_evaluation_{job_nb}")  
...
tokenizer = AutoTokenizer.from_pretrained(checkpoints_path, clean_up_tokenization_spaces=True)
model = BartForConditionalGeneration.from_pretrained(checkpoints_path)
...
# dataset = dataset.shuffle(seed=SEED).select(range(20)) #comment at the evaluate time
...
with open(os.path.join(results_dir, "len_and_txt.csv"), 'w', newline='') as file:
    writer = csv.writer(file)
...
with torch.no_grad():
    ...
    for _, batch in tqdm.tqdm(enumerate(loader, 0),desc=f'total iter: {len(loader)}', unit=" iter"):
        ...
        with open(os.path.join(results_dir, "len_and_txt.csv"), 'a', newline='') as file:
            writer = csv.writer(file)
